...
export class OpenRouterService {
  ...
  private buildPrompt(params: GenerateRouteParams): string {
    ...
    switch (params.routePriority) {
      case 'scenic':
        prompt += 'The route should be scenic, passing through beautiful landscapes, lakes, mountains, or coastline, with plenty of viewpoints and photo-worthy locations. Visual appeal and aesthetic experiences should be the priority. ';
        break;
      case 'twisty':
        prompt += 'The route should contain as many twisty, technical roads with curves and switchbacks that are enjoyable to ride on a motorcycle. Mountainous or hilly areas are preferred. The priority is riding pleasure, not the shortest path. ';
        break;
      case 'avoid_highways':
        prompt += 'The route should completely avoid highways (A1, A2, A4, etc.) and expressways (S1, S3, S7, etc.). Instead, prefer provincial and local roads with good surfaces. The priority is calm riding away from heavy traffic. ';
        break;
    }
    ...
    if (params.motorcycleType && params.motorcycleType !== 'other') {
      switch (params.motorcycleType) {
        case 'sport':
          prompt += 'The route is planned for a sport motorcycle, so good road surface quality and interesting curves are important. ';
          break;
        case 'cruiser':
          prompt += 'The route is planned for a cruiser, so riding comfort, wider roads, and nice views are preferred over technical handling. ';
          break;
        case 'touring':
          prompt += 'The route is planned for a touring motorcycle, so it should include diverse terrain and opportunities to visit interesting places. ';
          break;
        case 'adventure':
          prompt += 'The route is planned for an adventure motorcycle, so it may include sections of gravel and forest roads (but not difficult off-road), which are passable for this type of motorcycle. ';
          break;
        case 'standard':
          prompt += 'The route is planned for a standard motorcycle, so it should be balanced in terms of difficulty and road types. ';
          break;
        default:
